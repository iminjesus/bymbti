...
/* ── 문구 변형 ───────────────────────────────────────────────
 * 같은 질문이라도 실행할 때마다 다른 문장이 나오게 한다.
 * 결론(어느 쪽을 고르는지)은 그대로 두고 말만 바꾼다.
 */
let VARIANT_SEED = 0;
function setVariantSeed(n) { VARIANT_SEED = n | 0; }

function pickVariant(list, key) {
  if (!Array.isArray(list)) return list;
  return list[hashCode(`${key}|${VARIANT_SEED}`) % list.length];
}

/* 답을 시작하는 군더더기 한마디 (가끔만 붙는다) */
const OPENERS = ['', '', '', '', '음, ', '일단 ', '솔직히 ', '내 생각엔 '];
...
const CHOICE_LINE = {
  Ni: [
    '{PICK}. 지금 {OTHER} 고르면 이따 후회하는 그림이 벌써 보여.',
    '{PICK}. {OTHER} 쪽은 결국 같은 자리로 돌아올 것 같아.',
    '{PICK}. 이유는 설명하기 어려운데 그림이 그려져.',
  ],
  Ne: [
    '{PICK}… 아니 근데 {OTHER}도 아깝잖아. 둘 다 하는 방법 없어? 진짜 없어?',
    '{PICK}? {OTHER}? 아 몰라 둘 다 하면 안 되나 진짜.',
    '{PICK} 할게. 근데 {OTHER}랑 섞는 방법 분명 있을 텐데.',
  ],
  Si: [
    '{PICK}. 지난번에 {OTHER} 골랐다가 별로였던 거 기억 안 나?',
    '{PICK}. 전에 해봤는데 이쪽이 실패가 없었어.',
    '{PICK}. {OTHER}는 한 번 겪어봤잖아. 굳이 또?',
  ],
  Se: [
    '{PICK}. 지금 딱 그게 당겨. 딴 거 생각 안 나. 고민 끝.',
    '{PICK}. 몸이 그쪽이라는데 뭘 더 따져.',
    '{PICK}. 나 이미 정했어. 물어본 게 늦었어.',
  ],
  Ti: [
    '{PICK}. 따져보면 그게 더 합리적이야. 이유 세 개 있는데 들어볼래?',
    '{PICK}. 기준 잡고 비교하면 답 나와 있어.',
    '{PICK}. {OTHER}가 나은 조건이 뭔지 말해주면 다시 생각해볼게.',
  ],
  Te: [
    '{PICK}. 결정. 이거 고민하는 시간이 더 아까워. 다음.',
    '{PICK}. 정했다. 이견 있으면 지금 말해.',
    '{PICK}. 30초 썼으면 충분해. 넘어가자.',
  ],
  Fi: [
    '음… 나는 {PICK}인데, 네가 {OTHER} 하고 싶으면 그걸로 해도 돼. (진심 아님)',
    '나는 {PICK}. 근데 굳이 우기진 않을게. (우기고 싶음)',
    '{PICK}… 이라고 말은 했는데 다들 {OTHER}면 따를게. (서운함)',
  ],
  Fe: [
    '다들 뭐 골랐어? 난 많은 쪽으로 맞출게! …그래도 {PICK} 쪽이 좋긴 해.',
    '{PICK}? {OTHER}? 다들 의견 말해봐. 겹치는 쪽으로 가자.',
    '난 {PICK}인데, 반대하는 사람 있으면 바꿔도 돼!',
  ],
};
...
const AUX_TAIL = {
  Ni: ['…근데 이거 나중에 후회하는 거 아니지?', '…아 근데 뭔가 걸리는 게 있긴 해.', '어쨌든 방향은 맞다고 봐.'],
  Ne: ['아 아니면 딴 것도 한번 볼까?', '근데 세 번째 선택지 만들면 안 돼?', '아 방금 더 좋은 생각났는데.'],
  Si: ['전에도 이렇게 해서 괜찮았어.', '늘 하던 대로가 제일 안전해.', '기록해둘게. 다음에 또 쓰게.'],
  Se: ['말 나온 김에 나 이미 움직이고 있어.', '자, 그럼 바로 가자.', '더 얘기할 거 있어? 없지?'],
  Ti: ['왜 그런지 물어보면 설명해 줄게.', '반박할 거 있으면 들어볼게. 진심으로.', '논리 구멍 있으면 말해줘.'],
  Te: ['정했으면 바로 하자. 시간 아까워.', '담당이랑 순서만 정하면 끝이야.', '다음 안건.'],
  Fi: ['뭐, 네 마음 편한 대로 해도 되고.', '어차피 정답은 없잖아.', '나는 그냥 그렇다고.'],
  Fe: ['다들 괜찮지? 아니면 말해줘!', '혹시 불편한 사람 있으면 얘기해.', '다 같이 좋은 쪽으로 가자!'],
};
...
const LEAN_LINE = {
  Ni: ['{PICK}. 길게 보면 결국 그쪽으로 간다고 봐.', '{PICK}. 흐름이 이미 그쪽이야.', '{PICK}. 5년 뒤에 보면 답 나와 있을걸.'],
  Ne: ['{PICK}. {OTHER} 쪽 논리도 알겠는데, 그래도 나는 이쪽이야.', '{PICK}. {OTHER}도 일리는 있어. 근데 이쪽이 더 열려 있어.', '{PICK}. 반대 논리 다 들어봤고, 그래도 이쪽.'],
  Si: ['{PICK}. 지금까지 실제로 벌어진 걸 보면 그래.', '{PICK}. 사례를 보면 답이 나와 있어.', '{PICK}. 추측 말고 있었던 일로 판단하자.'],
  Se: ['{PICK}. 지금 눈앞에 보이는 게 그렇잖아.', '{PICK}. 현장 가보면 바로 알아.', '{PICK}. 이론 말고 실제로 그래.'],
  Ti: ['{PICK}. 전제부터 따져보면 이쪽이 맞아.', '{PICK}. 논리적으로 반대쪽이 성립을 안 해.', '{PICK}. 정의를 정확히 하면 이쪽밖에 없어.'],
  Te: ['{PICK}. 근거 대라면 세 개는 댈 수 있어.', '{PICK}. 숫자로 보면 명확해.', '{PICK}. 결론부터 말하면 이쪽이야.'],
  Fi: ['{PICK}. 이건 양보 못 해. 내 기준으로는 명확해.', '{PICK}. 이건 그냥 옳고 그름의 문제야.', '{PICK}. 다른 건 몰라도 이건 확실해.'],
  Fe: ['{PICK}. 사람들한테 어떤 영향이 가는지를 보면 그쪽이야.', '{PICK}. 누가 힘들어지는지를 보면 답 나와.', '{PICK}. 결국 사람이 받아들일 수 있어야 하잖아.'],
};
...
/* 축 × 진영별 논거.
   앞 4개는 T 계열(근거·숫자·비용), 뒤 4개는 F 계열(사람·마음·수용).
   같은 편을 들어도 T 는 T 답게, F 는 F 답게 근거를 대야 카드가 안 어색하다. */
const AXIS_VERDICT = {
  optimism: {
    pos: [
      '비관은 항상 구체적이고 낙관은 항상 늦게 증명돼.',
      '없어진 자리만 세고 새로 만들어진 자리는 아무도 안 세.',
      '단기랑 장기를 섞어 보니까 나빠 보이는 거야.',
      '파이가 커지면 나눠 먹을 자리도 같이 늘어나.',
      '전에도 다들 끝났다고 했는데 결국 다른 게 생겼잖아.',
      '사라지는 것만 보이지, 생기는 건 아직 이름이 없어서 안 보이는 거야.',
      '못 하던 걸 할 수 있게 되면 사람 손이 더 필요해져.',
      '지금 힘든 건 전환기라서 그래. 지나가는 구간이야.',
    ],
    neg: [
      '희망 섞지 말고 숫자만 보면 답 나와.',
      '전환기라고 하기엔 전환된 사례가 안 보여.',
      '기대는 매번 있었고 결과는 매번 늦었어.',
      '주변만 봐도 알아. 통계까지 갈 것도 없어.',
      '새 자리가 생겨도 잃은 사람이 그 자리로 못 가.',
      '속도가 문제야. 적응할 시간을 안 주잖아.',
      '좋아졌다는 건 평균이고, 평균은 사람을 못 살려.',
      '이미 그렇게 되고 있잖아. 눈앞에 있는 게 증거야.',
    ],
  },
  novelty: {
    pos: [
      '"원래 그랬다"는 건 근거가 아니야.',
      '기존 방식이 최적이라는 근거가 어디에도 없어.',
      '작게 먼저 돌려보면 위험도 같이 작아져.',
      '안 바꾸면 뒤처지는 것도 위험이야.',
      '지금 방식에 불편해하는 사람이 꽤 있잖아.',
      '안 해봤으니까 해보는 거지. 그게 재밌기도 하고.',
      '해보고 아니면 되돌리면 돼. 못 할 것도 없어.',
      '한 번은 갈아엎어야 할 때가 오는데, 지금이 그때야.',
    ],
    neg: [
      '전환 비용이 이득보다 커. 계산 끝났어.',
      '이거 바꾸느라 다른 게 다 밀려. 그게 진짜 비용이야.',
      '새것이 좋아 보이는 건 아직 문제가 안 터져서야.',
      '지금 굴러가고 있잖아. 굳이 위험을 살 이유가 없어.',
      '갑자기 바꾸면 적응 못 하는 사람이 생겨.',
      '전에 급하게 바꿨다가 어떻게 됐는지 다들 기억하잖아.',
      '되던 걸 왜 건드려. 익숙한 게 편한 사람도 있어.',
      '바꿀 거면 제대로 바꿔야지, 어설프게 바꾸면 더 나빠져.',
    ],
  },
  people: {
    pos: [
      '재교육 비용이 새로 뽑는 비용보다 싸. 숫자로도 이쪽이야.',
      '남는 사람들 사기가 떨어지면 그게 더 큰 손실이야.',
      '길게 보면 사람 잃은 조직이 살아남은 적이 없어.',
      '잘라내는 건 언제든 할 수 있어. 되돌리는 게 안 돼.',
      '이건 양보 못 해. 사람이 먼저야.',
      '당장 직접 피해 보는 사람이 눈에 보이잖아.',
      '숫자가 좋아져도 사람이 무너지면 결국 다 무너져.',
      '효율 얘기 전에 사람 얘기부터 하는 게 순서잖아.',
    ],
    neg: [
      '감정 빼고 보면 효율이 답이야. 냉정하게 가자.',
      '자원이 한정돼 있으면 효율이 곧 생존이야.',
      '경쟁자는 이미 하고 있어. 우리만 안 하면 답이 없어.',
      '버티는 건 해결이 아니라 미루는 거야.',
      '다 지키려다 다 잃는 경우를 너무 많이 봤어.',
      '온정으로 결정하면 나중에 더 크게 잘라내게 돼.',
      '지금 아끼는 비용으로 다른 자리를 만들 수도 있잖아.',
      '현실적으로 안 하면 우리가 먼저 도태돼.',
    ],
  },
  action: {
    pos: [
      '기다리는 비용이 실행 비용보다 커.',
      '지연도 결정이야. 그것도 비용이 붙어.',
      '움직여야 정보가 생겨. 앉아서는 안 나와.',
      '조건 다 맞을 때까지 기다리면 아무것도 못 해.',
      '망설이는 동안 마음만 상해. 그냥 하자.',
      '하다 보면 길이 보여. 완벽한 시점은 안 와.',
      '지금 할 수 있으면 지금 하는 게 맞아.',
      '일단 시작하면 절반은 끝난 거야.',
    ],
    neg: [
      '정보가 부족한 상태의 결정은 도박이야.',
      '되돌릴 수 없는 결정은 최대한 늦게 하는 게 맞아.',
      '준비 안 된 상태로 하면 사고 나. 순서대로 가자.',
      '한 박자 늦게 가는 게 오히려 빠를 때가 있어.',
      '지금 하면 누군가는 상처받아. 시간이 좀 필요해.',
      '아직 다들 마음의 준비가 안 됐어.',
      '전에 급하게 했다가 어떻게 됐는지 기억나잖아.',
      '지금은 때가 아니야. 좀 더 보고 움직이자.',
    ],
  },
};

/* 유형마다 붙는 말버릇 — 같은 논거를 대도 카드가 다르게 읽히게 */
const TYPE_QUIRK = {
  ISTJ: ['자료 찾아서 보여줄게.', '출처는 내가 정리해둘게.'],
  ISFJ: ['근데 다들 괜찮으면 나도 괜찮아.', '무리하는 사람 없었으면 좋겠어.'],
  INFJ: ['설명하긴 어려운데 그냥 알아.', '나중에 보면 내 말이 맞을걸.'],
  INTJ: ['이미 시나리오 세 개 돌려봤어.', '반박은 논리로만 받을게.'],
  ISTP: ['말보다 그냥 해보면 알아.', '나는 결과만 보면 돼.'],
  ISFP: ['근데 강요하는 건 아니야.', '내 취향이 그렇다는 거야.'],
  INFP: ['이건 좀 중요한 문제라고 생각해.', '나만 이렇게 느끼나 싶기도 하고.'],
  INTP: ['반례 있으면 진짜로 말해줘.', '전제가 틀렸으면 결론도 틀린 거고.'],
  ESTP: ['말 나온 김에 지금 해보자.', '해보면 5분이면 알아.'],
  ESFP: ['어차피 인생 뭐 있어.', '재밌으면 된 거 아니야?'],
  ENFP: ['아 근데 딴 얘기지만.', '갑자기 다른 생각났는데 나중에 말할게.'],
  ENTP: ['반대로 물어볼게. 아니라는 근거는 뭔데?', '이거 토론하면 재밌겠다.'],
  ESTJ: ['그래서 언제까지 정할 거야?', '담당 정하고 넘어가자.'],
  ESFJ: ['다른 사람들 의견도 들어봐야 하지 않을까?', '내가 물어보고 올게.'],
  ENFJ: ['다들 납득할 수 있는 쪽으로 가자.', '너 생각은 어때? 진짜 궁금해.'],
  ENTJ: ['결론 났으면 실행 얘기 하자.', '이견 있으면 지금 말해.'],
};

/* 진영 안에서 순번을 돌려 논거가 겹치지 않게 고른다.
   T 유형은 앞 절반(근거·비용), F 유형은 뒤 절반(사람·수용)에서 가져온다. */
function pickByRank(list, rank, isT) {
  const half = list.length / 2;
  const base = isT ? 0 : half;
  return list[base + ((Math.abs(rank) + VARIANT_SEED) % half)];
}
